...
export class SchemaParser {
      ...
      for (const declaration of node.declarationList.declarations) {
        if (ts.isVariableDeclaration(declaration) && declaration.initializer) {
          ...
          if (tableMetadata) {
            ...
            const tableRelations = tableMetadata.constraints
              .filter((c): c is RelationConstraint => c.type === "relation");
  ...
  private extractTableFromDeclaration(declaration: ts.VariableDeclaration): TableMetadata | null {
    ...
    const constraints = this.extractConstraintsFromExpression(declaration.initializer);
  ...
  private findTableCall(node: ts.Node): ts.CallExpression | null {
    if (ts.isCallExpression(node)) {
      ...
      if (ts.isIdentifier(node.expression) && node.expression.text === "Table") {
        return node;
      }
    ...
    if (ts.isObjectLiteralExpression(node)) {
      for (const property of node.properties) {
        if (ts.isPropertyAssignment(property) && ts.isIdentifier(property.name)) {
          fields[property.name.text] = this.extractValidatorType(property.initializer);
        }
      }
    }
  ...
  private parseUniqueConstraint(node: ts.CallExpression): UniqueConstraint | null {
  ...
  private parseRelationConstraint(node: ts.CallExpression): RelationConstraint | null {
    const fieldArg = node.arguments[0];
  ...
  private parseIndexConstraint(node: ts.CallExpression): IndexConstraint | null {
    ...
    const name = nameArg ? this.extractStringLiteral(nameArg) : undefined;
